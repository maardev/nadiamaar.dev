...
export function ProgettiClient({ pageData }: { pageData?: ProgettiPageData | null }) {
  const [active, setActive] = useState('Tutti')

  // Merge Payload projects with fallback to hardcoded default data
  const allProjects = pageData?.projects?.length
    ? pageData.projects.map((p) => ({
        title: p.title,
        category: p.category ?? '',
        description: p.description ?? '',
        tags: (p.tags ?? []).map((t) => t.tag),
        gradient: p.gradient ?? 'from-violet-500/20 to-fuchsia-500/10',
        url: p.url ?? null,
      }))
    : projects

  const filtered = active === 'Tutti' ? allProjects : allProjects.filter((p) => p.category === active)
  const dynCategories = ['Tutti', ...Array.from(new Set(allProjects.map((p) => p.category).filter(Boolean)))]
