...
// Hardcoded fallback — used when Payload record doesn't exist yet
const defaultPlans: NonNullable<PrezziPageData['plans']> = [
  {
    name: 'Starter',
    price: '990',
    period: 'una tantum',
...
export function PrezziClient({ pageData }: { pageData?: PrezziPageData | null }) {
  const plans = pageData?.plans?.length ? pageData.plans : defaultPlans

  return (
    <div
      className="min-h-screen bg-[#0a0a0f] text-white"
      data-block-id="page-prezzi"
      data-block-type="static-page"
    >
      {/* ── Hero ── */}
        ...
        <div className="relative mx-auto max-w-3xl text-center">
          ...
          >
            <span className="bg-gradient-to-r from-white via-white to-violet-300 bg-clip-text text-transparent">
              {pageData?.heroTitle ?? 'Scegli il piano giusto'}
            </span>
          </motion.h1>
          <motion.p
            initial="hidden"
              ...
              >
                ...
                <ul className="mt-6 flex-1 space-y-3">
                  {(plan.features ?? []).map((feat) => (
                    <li
                      key={typeof feat === 'string' ? feat : feat.text}
